...
const backupFilePath = './src/data/games.backup.json';
...
try {
  ...
  // 创建备份
  fs.writeFileSync(backupFilePath, JSON.stringify(existingGames, null, 2));
  console.log('✅ 已创建备份文件');
  ...
  originalGames.forEach(game => {
    const exists = updatedGames.some(g => g.slug === game.slug);
    if (!exists) {
      updatedGames.unshift(game); // 添加到开头
      addedCount++;
      console.log(`✅ 已添加: ${game.title}`);
    } else {
      console.log(`⚠️  已存在: ${game.title}`);
    }
  });
